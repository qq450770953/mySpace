...
def create_project():
    ...
    try:
        # 获取当前登录用户
        current_user_id = get_jwt_identity()
        logger.info(f"创建新项目 - 用户ID: {current_user_id}")
    ...
    try:
        ...
        if 'start_date' in data and data['start_date']:
            try:
                ...
                logger.warning(f"无效的开始日期格式: {data['start_date']}, 错误: {str(e)}")
                # 尝试多种日期格式
                try:
                    project.start_date = datetime.strptime(data['start_date'], '%Y-%m-%d')
                except ValueError:
                    logger.error(f"无法解析开始日期: {data['start_date']}")
                    return jsonify({'error': f'无效的开始日期格式: {data["start_date"]}'}), 400
        ...
        if 'end_date' in data and data['end_date']:
            try:
                ...
                logger.warning(f"无效的结束日期格式: {data['end_date']}, 错误: {str(e)}")
                # 尝试多种日期格式
                try:
                    project.end_date = datetime.strptime(data['end_date'], '%Y-%m-%d')
                except ValueError:
                    logger.error(f"无法解析结束日期: {data['end_date']}")
                    return jsonify({'error': f'无效的结束日期格式: {data["end_date"]}'}), 400
        ...
        if 'manager_id' in data and data['manager_id'] != project.manager_id:
            ...
            new_manager = User.query.get(data['manager_id'])
            changes.append(f'负责人从 {old_manager.name if old_manager else "无"} 改为 {new_manager.name if new_manager else "无"}')
...
def delete_project(project_id):
    ...
    try:
        ...
        if not (project.owner_id == current_user_id or 
            ...
            return jsonify({'error': '没有权限删除此项目'}), 403
        
        # 检查是否有未完成的任务
        if project.tasks.filter(Task.status != 'completed').count() > 0:
            return jsonify({'error': '无法删除包含未完成任务的项目，请先完成或删除所有任务'}), 400
...
def remove_project_member(project_id, user_id):
    ...
    try:
        ...
        if project.tasks.filter(Task.assignee_id == user_id, Task.status != 'completed').count() > 0:
            return jsonify({'error': '无法移除有未完成任务的项目成员'}), 400
...
def detail(project_id):
    ...
    try:
        ...
        if bypass_jwt:
            logger.info(f"JWT bypass enabled for project detail - 使用测试用户")
            ...
            if not current_user_id:
                logger.warning("Unauthorized access to project detail page", extra={
                    'ip': request.remote_addr,
                    'user_agent': request.user_agent.string
                })
                return redirect(url_for('auth.login_page'))
        
        logger.info(f"获取项目 {project_id} 详情页面 - 用户ID: {current_user_id}")
        ...
        # bypass_jwt模式下跳过权限检查
        if not bypass_jwt and not (user_is_owner or user_is_manager or user_is_member):
            ...
            if field in data:
                ...
                if field in ['start_date', 'end_date'] and data[field]:
                    try:
                        ...
                        return jsonify({'error': f'无效的日期格式: {field}'}), 400
                else:
    ...
    try:
        ...
        if method == 'GET':
            ...
            try:
                ...
                logger.error(f"保存项目 {project_id} 更新时出错: {str(e)}")
                db.session.rollback()
                return jsonify({'error': f'保存更新时出错: {str(e)}'}), 500
            ...
            try:
                ...
                logger.error(f"删除项目 {project_id} 时出错: {str(e)}")
                db.session.rollback()
                return jsonify({'error': f'删除项目时出错: {str(e)}'}), 500
        
        # 不支持的HTTP方法
        return jsonify({'error': f'不支持的HTTP方法: {method}'}), 405
        
    except Exception as e:
        logger.error(f"处理项目 {project_id} API请求时出错: {str(e)}")
        return jsonify({'error': str(e)}), 500
    ...
    try:
        ...
        if data.get('start_date'):
            try:
                ...
                logger.warning(f"无效的开始日期格式: {data.get('start_date')}, 错误: {str(e)}")
                # 如果日期格式无效，使用当前日期
        ...
        if data.get('end_date'):
            try:
                ...
                logger.info(f"解析的结束日期: {end_date}")
            except (ValueError, TypeError) as e:
                logger.warning(f"无效的结束日期格式: {data.get('end_date')}, 错误: {str(e)}")
        ...
        if manager_id:
            try:
                ...
                logger.warning(f"无效的manager_id格式: {manager_id}，设置为None")
                manager_id = None
        else:
            logger.info("未提供manager_id，设置为当前用户")
            # 如果没有提供manager_id，设置为当前用户（创建者）
        ...
        try:
            ...
            logger.warning(f"添加项目成员出错，可能是项目成员模型不存在: {str(e)}")
            # 继续执行，不中断流程
...
def get_project_managers():
    ...
    try:
        ...
        if not bypass_jwt:
            current_user_id = get_jwt_identity()
            if not current_user_id:
                logger.warning("未授权访问，未找到用户身份")
                return jsonify({'error': '未授权访问，请登录'}), 401
        
        # 获取所有用户
        try:
            all_users = User.query.filter(User.is_active == True).all()
            logger.info(f"已找到 {len(all_users)} 个活跃用户用于项目经理选择")
            
            if not all_users or len(all_users) == 0:
                logger.warning("没有找到活跃用户，返回默认项目经理列表")
                return jsonify({
                    'success': True,
                    'project_managers': [
                        {'id': 1, 'name': '管理员 (默认)'},
        ...
        for user in sorted_users:
            ...
            if 'admin' in user_roles or 'project_manager' in user_roles or 'manager' in user_roles:
                if 'admin' in user_roles:
                    display_name = f"{display_name} (管理员)"
                else:
        ...
        logger.info(f"返回 {len(project_managers)} 个可选的项目负责人")
...
def get_project_members_api(project_id):
    """获取项目成员的API端点，无需认证，用于前端获取成员列表"""
    try:
        logger.info(f"获取项目 {project_id} 的成员列表")
        ...
        if not members:
            ...
            if project.manager_id:
                ...
                if manager:
                    members.append({
                        'id': manager.id,
                        'name': manager.name or manager.username or f"负责人 #{manager.id}",
                        'role': 'manager'
                    })
        
        # 如果仍然没有找到任何成员，获取系统中的活跃用户作为备选
        if not members:
        ...
        logger.info(f"返回项目 {project_id} 的 {len(members)} 个成员")
